 ...
 * Author: Reliance
 *
 * Created on 27 August, 2024, 9:13 PM
 */
...
void putch(unsigned char byte) //transmit one byte of data
{
	/* Output one byte */
	/* Set when register is empty */
	while(!TXIF) //TXIF = 0 (FULL)
	{
		continue;
	}
	TXREG = byte;
...
unsigned char getch(void) //receive one byte
{
	...
	while(!RCIF) //RCIF = 0(EMPTY))
	{
		continue;
	}
	return RCREG;
}
//micro controller receives one byte of data and again transmit the same data to source
