 ...
 * Author: Reliance
 *
 * Created on 12 August, 2024, 2:13 PM
 */
...
{
	CLCD_RS = control_bit; //assign the control bit to RC1
	CLCD_PORT = byte;    //
	...
	PORT_DIR = INPUT;  //TRISD7=1 we can use this also
	CLCD_RW = HI;
	CLCD_RS = INSTRUCTION_COMMAND;  

	do
	{
        //we are using clock pulse
		CLCD_EN = HI;
		CLCD_EN = LO;
	} while (CLCD_BUSY); //we have to continous moniter D7 =0 or not
              // 0/1
